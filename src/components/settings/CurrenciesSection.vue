...
<template>
  <section class="bg-surface rounded-xl border border-border p-3 sm:p-5">
    <div class="flex items-center justify-between gap-2 mb-3">
      <h2 class="text-base sm:text-lg font-semibold text-text-primary shrink-0">{{ t('currencies') }}</h2>
    ...
    <div class="space-y-1.5 max-h-72 overflow-y-auto overflow-x-hidden">
      <div v-for="(c, idx) in filteredCurrencies" :key="c.id" class="flex gap-2 items-center rounded-lg px-2 py-1" :class="c.id === settingsStore.settings.mainCurrencyId ? 'bg-primary-light/50' : ''">
        ...
        <Tooltip :text="t('set_as_primary')">
          <button @click="setMainCurrency(c.id)" class="p-1 rounded-lg transition-colors shrink-0" :class="c.id === settingsStore.settings.mainCurrencyId ? 'text-yellow-500' : 'text-text-muted hover:text-yellow-500'">
