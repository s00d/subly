...
export async function getDashboardCharts(): Promise<DashboardChartsDto> {
  return invoke("get_dashboard_charts");
}

export async function getDashboardForecast(): Promise<DashboardForecastDto> {
  return invoke("get_dashboard_forecast");
}

export async function getDashboardTrends(): Promise<DashboardTrendsDto> {
  return invoke("get_dashboard_trends");
}

export async function getRateHistoryWidget(targetIds: string[], days: number): Promise<Record<string, RateHistoryPoint[]>> {
  return invoke("get_rate_history_widget", { targetIds, days });
}
